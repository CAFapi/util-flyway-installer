...
*   `db.setCollation`: (Optional) Specifies whether to set C collation when creating the database. Valid options are: [true, false]. Default is false.
...
For example:
    ...
    java -cp "*:db" com.github.cafapi.util.flywayinstaller.Application -db.host localhost -db.port 5437 -db.user postgres -db.secretKeys CAF_SERVICE_DATABASE_PASSWORD -db.name test-db2 -db.schema test_schema -db.setCollation true  -log DEBUG
